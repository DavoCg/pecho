...
module.exports = {
    getPlaces: function(query, callback){
        ...
        Place
            ...
            .exec(function(err, result){
                ...
                /**
                 * Augment each places found and if no err return callback(null, augmentedPlaces)
                 */
                async.map(places, augmentPlace, callback);

                /**
                 * Add distance + matching infos to place
                 * @param place
                 * @param cb
                 */
    ...
    addPlace: function(place, callback){
        if(!place.name) return callback(new Error('Please specify a name for the place'));
        if(!Array.isArray(place.hashtags)) return callback(new Error('Hashtags not an array'));
        if(!Array.isArray(place.location)) return callback(new Error('Location not an array'));
